...
GPP_ERR_RE = re.compile(r"(?:^|/|\\)main\.cpp:(\d+):(?:(\d+):)?\s*(?:fatal error|error|warning):\s*(.*)")


def _write_main_cpp(tmp_dir: str, code: str) -> str:
    path = os.path.join(tmp_dir, "main.cpp")
    with open(path, "w", encoding="utf-8") as f:
        f.write(code)
    return path


def lint_cpp(code: str) -> Dict[str, Any]:
    with tempfile.TemporaryDirectory() as tmp:
        file_path = _write_main_cpp(tmp, code)

        try:
            proc = subprocess.run(
                ["g++", "-std=c++17", "-Wall", "-Wextra", "-fsyntax-only", file_path],
                capture_output=True,
                text=True,
        ...
        return {"errors": _parse_gpp_errors(proc.stderr)}
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            compile_proc = subprocess.run(
                ["g++", "-std=c++17", "-Wall", "-Wextra", file_path, "-o", binary_path],
                capture_output=True,
                text=True,
                timeout=COMPILE_TIMEOUT_SEC,
            )
...
def _parse_gpp_errors(stderr: str) -> list[Dict[str, Any]]:
    errors = []
    for line_text in stderr.splitlines():
        match = GPP_ERR_RE.search(line_text)
        if match:
            errors.append({
    ...
    if not errors and stderr.strip():
        errors.append({"line": 0, "col": None, "message": stderr.strip()})
